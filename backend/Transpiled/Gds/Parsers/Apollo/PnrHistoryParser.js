...
class PnrHistoryParser {
	...
	static parseRcvdLine($line) {
		...
		if (StringUtil.startsWith($line, 'RCVD-')) {
	...
	static parsePricingLine($line) {
		let $wrappedLines, $parsedLines, $wrappedLine, $parsed, $matches, $content;
		$wrappedLines = StringUtil.lines(StringUtil.wrapLinesAt($line, 64));
		if (php.count($wrappedLines) > 1) {
			$parsedLines = [];
			for ($wrappedLine of Object.values($wrappedLines)) {
				if ($parsed = (this.parsePricingLine($wrappedLine) || [])[0]) {
	...
	static parseCompleteDump($dump) {
		...
		if (!($header = this.parseHistoryHeaderLine($line = php.array_shift($lines)))) {
			$line && php.array_unshift($lines, $line);
		}
		$rcvdList = [];
		$rcvd = null;
		$unexpected = [];
