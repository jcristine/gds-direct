...
class SabrePricingParser {
	...
	static splitLinesByPattern(lines, pattern, includeDelimiter) {
		const idxs = php.array_keys(Fp.filter((line) => {
			return php.preg_match('/' + pattern + '/', line);
		}, lines));

		const result = [];
		for (const i of Object.values(php.array_reverse(idxs))) {
	...
	static parse(dump) {
		...
		let lines = StringUtil.lines(dump);
		const endLine = php.array_pop(lines);
		if (endLine.trim() !== '.') {
			lines.push(endLine);
		}
		let dataExistsInfo;
		...
		const sections = php.array_filter(this.splitLinesByPattern(lines, '^\\s*$'));
		let firstSection = php.array_shift(sections);
		const wasPqRetained = php.trim(php.implode(php.PHP_EOL, firstSection)) === 'PRICE QUOTE RECORD RETAINED';
		if (wasPqRetained) {
