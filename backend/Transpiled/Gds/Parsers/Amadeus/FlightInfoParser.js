...
class FlightInfoParser
{
    ...
    static parseLocationLine($line)  {
        ...
        $meals = php.array_map((...args) => this.parseMeal(...args), php.preg_split(/\s+/, $split['M']));
        $result = {
        ...
        $hasMainPart = !Fp.any('is_null', $meals)
    ...
    static locationsToLegs($locations)  {
        ...
        for ($location of Object.values($locations)) {
            if (php.array_keys(php.array_filter($location)) === ['meals']) {
    ...
    static operationsToLegs($operations)  {
        let $legs, $departureStarted, $currentLeg, $currentTz, $currentDeparture, $operation, $startsDeparture, $tz, $transformed;
    ...
    static parseAdditionalInfo($lines)  {
        ...
        while (!php.is_null($line = php.array_shift($lines))) {
            if (php.trim($line) === 'COMMENTS-') {
                [$comments, $lines] = this.parseSequence($lines, (...args) => this.parseCommentLine(...args));
        ...
        for ($i = php.count($comments) - 1; $i > 0; --$i) {
            if (php.array_keys(php.array_filter($comments[$i])) === ['raw']) {
    ...
    static parsePlannedSegment($lines)  {
        ...
        [$locations, $lines] = this.parseSequence($lines, (...args) => this.parseLocationLine(...args));
        ...
        return {
            ...
            'legs': this.locationsToLegs($locations),
    ...
    static parse($dump)  {
        ...
        $segments = [];
        while ($linesLeft) {
            $segment = this.parsePlannedSegment($linesLeft) || this.parseOperationalSegment($linesLeft) || this.parseNotOperationalSegment($linesLeft) || this.parseFlownSegment($linesLeft);
            if ($segment) {
