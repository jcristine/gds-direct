...
return $rec['ptc'];}, $modPtcs));
    ...
    static transformTicketInvoiceInfo($ticketList, $history, $reservationDt)  {
        let $records, $error, $getNum, $numToTrans, $rcvdRecord, $actRecord, $parsed, $ticketNum, $trans, $isVoided, $ticket;
    ...
    static collectPqFares($pqSegments)  {
        let $fareList, $rSegNums, $departureAirport, $fareBasis, $destinationAirport, $componentNumber, $segmentNumber, $i, $pqSeg, $next;
    ...
    static transformBaggageInfo($baggageData, $ptc)  {
        ...
        return {
            ...
            'carryOnAllowanceBlock': {'segments': Fp.flatten(Fp.map(($carryOnInfo) => {
