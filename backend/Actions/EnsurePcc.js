...
const makeRejection = (cmdRec, pcc, gds) => {
	...
	if (gds === 'apollo') {
		...
		} else if (cmdRec.output.startsWith('ERR: INVALID ACCOUNT - APOLLO')
				|| cmdRec.output.startsWith('ERR: NO ACCOUNT REC SET UP - APOLLO')
				|| cmdRec.output.startsWith('ERR: INVALID EMULATION LOCATION')
		) {
			rejection = Rej.Forbidden;
