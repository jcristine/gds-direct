...
const logOutput = async (rqBody, session, whenCmdRqId, output) => {
	const cmdRqId = await whenCmdRqId;
...
const handleCmsExc = (exc) => {
	let type = null;
	const data = Debug.getExcData(exc);
	const excStr = (exc + '').slice(0, 2000);
...
exports.logProcess = async ({params, whenCmdRqId, whenCmsResult}) => {
	...
	const duration = ((Date.now() - calledDtObj.getTime()) / 1000).toFixed(3);
	return whenCmdRqId.then(async cmdRqId => {
		const prevCmdRqRow = await Db.with(db => db.fetchOne({
			table: 'cmd_rq_log',
			where: [
				['id', '<', cmdRqId],
				['sessionId', '=', session.id],
