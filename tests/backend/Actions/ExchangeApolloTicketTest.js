...
class ExchangeApolloTicketTest extends require('../Transpiled/Lib/TestCase.js')
{
			...
			initialCommands: [
				{
				    ...
				    "state": {"area":"A","pcc":"2F3K","record_locator":"PZWSV5","can_create_pq":false,"has_pnr":true,"is_pnr_stored":true,"cmdType":"redisplayPnr","scrolledCmd":"*R","pricing_cmd":null}
				},
				{
				    "cmd": "MR",
				    "output": [
	...
	async test_inputHbFexMask(testCase) {
		let stateful = await GdsDirectDefaults.makeStatefulSession('apollo', testCase.input, {
			initialState: {
				"area":"A","pcc":"2F3K","record_locator":"PZWSV5","can_create_pq":false,"has_pnr":true,
